...
$f5Key1SecretNamePrefix = 'f5-nva-license-key-' 
...
if($null -eq $kv)
    {
      ...
      $licenseKey1 = ConvertTo-SecureString (Read-Host "Enter F5 license key #1") -AsPlainText -Force
      $licenseKey2 = ConvertTo-SecureString (Read-Host "Enter F5 license key #2") -AsPlainText -Force
      Set-AzKeyVaultSecret -VaultName $kvName -Name $($f5Key1SecretNamePrefix + '1') -SecretValue $licenseKey1
      Set-AzKeyVaultSecret -VaultName $kvName -Name $($f5Key1SecretNamePrefix + '2') -SecretValue $licenseKey2
    }
else
    {
